...
pub async fn list(
    ...
    let accounts = if c.role == "agency_admin" {
        sqlx::query_as::<_, Account>("SELECT * FROM tenants WHERE is_active=true ORDER BY name")
            .fetch_all(&s.db)
            .await?
    } else {
        sqlx::query_as::<_, Account>("SELECT * FROM tenants WHERE id=$1 AND is_active=true")
            .bind(aid)
            .fetch_all(&s.db)
            .await?
    };
...
pub async fn create(
    State(s): State<AppState>,
    Extension(c): Extension<Claims>,
    Json(r): Json<CreateAccountRequest>,
) -> ApiResult<impl IntoResponse> {
...
pub async fn get(
    ...
    Ok(Json(json!(sqlx::query_as::<_, Account>(
...
pub async fn update(
    ...
    Extension(c): Extension<Claims>,
    Path(id): Path<Uuid>,
    Json(r): Json<UpdateAccountRequest>,
) -> ApiResult<impl IntoResponse> {
    let aid = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    if c.role != "agency_admin" && aid != id {
        return Err(AppError::Forbidden);
...
pub async fn delete(
    ...
    let r = sqlx::query("DELETE FROM tenants WHERE id=$1")
        .bind(id)
        .execute(&s.db)
        .await?;
...
pub async fn get_settings(
    ...
    let settings = sqlx::query_scalar::<_, Option<serde_json::Value>>(
        "SELECT settings FROM tenants WHERE id=$1",
    )
    .bind(id)
    .fetch_optional(&s.db)
    .await?
    .ok_or(AppError::NotFound(format!("Account {id} not found")))?;
    Ok(Json(
        json!({"settings": settings.unwrap_or(serde_json::Value::Object(Default::default()))}),
    ))
}
...
pub async fn update_settings(
    ...
    Ok(Json(
        json!({"message":"Settings updated","settings": settings}),
    ))
}
