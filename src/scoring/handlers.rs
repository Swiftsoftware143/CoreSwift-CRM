...
pub async fn list_rules(
    ...
    Ok(Json(
        json!({"rules": sqlx::query_as::<_,ScoreRule>("SELECT * FROM score_rules WHERE tenant_id=$1 ORDER BY name").bind(t).fetch_all(&s.db).await?}),
    ))
}
...
pub async fn create_rule(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    if r.name.is_empty() || r.event_type.is_empty() {
        return Err(AppError::Validation("Name and event_type required".into()));
    }
...
pub async fn delete_rule(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let r = sqlx::query("DELETE FROM score_rules WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
    if r.rows_affected() == 0 {
        return Err(AppError::NotFound(format!("Score rule {id} not found")));
    }
...
pub async fn get_score(
    ...
    .ok_or(AppError::NotFound(format!(
        "Score for contact {cid} not found"
    )))?)))
}
...
pub async fn calculate_score(
    ...
    Extension(c): Extension<Claims>,
    Path(cid): Path<Uuid>,
    Json(r): Json<ScoreEventRequest>,
) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    if r.event_type.is_empty() {
        return Err(AppError::Validation("event_type required".into()));
    }
    Ok(Json(json!(
        engine::calculate_score(&s.db, t, cid, &r.event_type).await?
    )))
}
...
pub async fn list_thresholds(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    Ok(Json(
        json!({"thresholds": sqlx::query_as::<_,ScoringThreshold>("SELECT * FROM scoring_thresholds WHERE tenant_id=$1 ORDER BY min_score").bind(t).fetch_all(&s.db).await?}),
    ))
}
...
pub async fn create_threshold(
    State(s): State<AppState>,
    Extension(c): Extension<Claims>,
    Json(r): Json<CreateThresholdRequest>,
) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    if r.min_score < 0 {
        return Err(AppError::Validation("min_score must be >= 0".into()));
    }
...
pub async fn delete_threshold(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let r = sqlx::query("DELETE FROM scoring_thresholds WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
    if r.rows_affected() == 0 {
        return Err(AppError::NotFound(format!(
            "Scoring threshold {id} not found"
        )));
    }
    Ok(Json(json!({"message":"Deleted"})))
}
...
pub async fn list_webhooks(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    Ok(Json(
        json!({"webhooks": sqlx::query_as::<_,ScoringWebhook>("SELECT * FROM scoring_webhooks WHERE tenant_id=$1 ORDER BY name").bind(t).fetch_all(&s.db).await?}),
    ))
}
...
pub async fn delete_webhook(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let r = sqlx::query("DELETE FROM scoring_webhooks WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
