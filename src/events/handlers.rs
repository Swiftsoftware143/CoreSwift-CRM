...
pub async fn ingest(
    ...
    tokio::spawn(async move {
        dispatcher::dispatch_automation(
            &db_clone,
            tid_clone,
            &et,
            ent_t.as_deref(),
    ...
    crate::audit::logger::log_event(
        &s.db,
        tid,
        Some(Uuid::parse_str(&c.sub).map_err(|_| AppError::Unauthorized)?),
        &format!("event.ingested.{}", source),
        "event",
        Some(event.id),
        Some(json!({"event_type": body.event_type, "source": source})),
        None,
    )
    .await;
...
pub async fn ingest_get(
    ...
    Ok(Json(
        json!({"status": "ready", "source": source, "message": "Webhook endpoint active. POST events here."}),
    ))
}
...
pub async fn list_events(
    ...
    Ok(Json(
        json!({"events": events, "total": total, "page": page, "per_page": per_page}),
    ))
}
...
pub async fn list_delayed(
    ...
    Ok(Json(
        json!({"delayed_actions": actions, "total": total, "page": page, "per_page": per_page}),
    ))
}
...
pub async fn schedule_delayed(
    ...
    if !["timeout", "no_event", "no_action"].contains(&r.condition_type.as_str()) {
        return Err(AppError::Validation(
            "condition_type must be 'timeout', 'no_event', or 'no_action'".to_string(),
        ));
    }
