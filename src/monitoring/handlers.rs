...
pub async fn get_health(
    ...
    match health {
        ...
        None => Ok(Json(
            json!({"score": 100, "risk_level": "healthy", "message": "No signals recorded yet"}),
        )),
    }
}

/// POST /api/monitoring/health — Record a health signal
pub async fn update_health_signal(
    State(s): State<AppState>,
    Extension(c): Extension<Claims>,
    Json(r): Json<SignalRequest>,
) -> ApiResult<impl IntoResponse> {
    let tid = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    ...
    Ok(Json(
        json!({"message": "Signal recorded", "signal": r.signal, "entity": r.entity_id}),
    ))
}
...
pub async fn create_threshold(
    ...
    if !["lt", "gt", "eq", "lte", "gte"].contains(&r.operator.as_str()) {
        return Err(AppError::Validation(
            "operator must be lt, gt, eq, lte, or gte".to_string(),
        ));
    }
...
pub async fn update_threshold(
    ...
    Extension(c): Extension<Claims>,
    Path(id): Path<Uuid>,
    Json(r): Json<UpdateThresholdRequest>,
) -> ApiResult<impl IntoResponse> {
    let tid = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let t = sqlx::query_as::<_, HealthThreshold>(
...
pub async fn delete_threshold(
    ...
    let tid = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    sqlx::query("DELETE FROM health_thresholds WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(tid)
        .execute(&s.db)
        .await?;
