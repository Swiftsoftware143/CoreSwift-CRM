...
pub async fn list_categories(
    ...
    Ok(Json(
        json!({"categories": sqlx::query_as::<_,TagCategory>("SELECT * FROM tag_categories WHERE tenant_id=$1 ORDER BY name").bind(t).fetch_all(&s.db).await?}),
    ))
}
pub async fn create_category(
    ...
    let cat = sqlx::query_as::<_, TagCategory>(
        "INSERT INTO tag_categories(id,tenant_id,name,color) VALUES($1,$2,$3,$4) RETURNING *",
    )
    .bind(Uuid::new_v4())
    .bind(t)
    .bind(&r.name)
...
pub async fn delete_category(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let r = sqlx::query("DELETE FROM tag_categories WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
...
pub async fn list_tags(
    ...
    let tags = sqlx::query_as::<_, Tag>(
        "SELECT * FROM tags WHERE tenant_id=$1 AND is_active=true ORDER BY name",
    )
    .bind(t)
    .fetch_all(&s.db)
    .await?;
    ...
    for row in contact_rows {
        if let Ok(id) = row.try_get::<Uuid, _>(0) {
            let c: i64 = row.get(1);
            contact_counts.insert(id, c);
        }
    }
    ...
    for row in integration_rows {
        if let Ok(id) = row.try_get::<Uuid, _>(0) {
            let c: i64 = row.get(1);
            integration_counts.insert(id, c);
        }
    }

    let result: Vec<TagWithCounts> = tags
        .into_iter()
        .map(|tag| TagWithCounts {
            id: tag.id,
...
pub async fn get_tag(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    Ok(Json(json!(sqlx::query_as::<_, Tag>(
    ...
    .fetch_optional(&s.db)
    .await?
    .ok_or(AppError::NotFound(format!("Tag {id} not found")))?)))
}
pub async fn update_tag(
    State(s): State<AppState>,
    Extension(c): Extension<Claims>,
    Path(id): Path<Uuid>,
    Json(r): Json<UpdateTagRequest>,
) -> ApiResult<impl IntoResponse> {
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
...
pub async fn delete_tag(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let r = sqlx::query("DELETE FROM tags WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
...
pub async fn assign_tag(
    State(s): State<AppState>,
    Extension(c): Extension<Claims>,
    Json(r): Json<AssignTagRequest>,
) -> ApiResult<impl IntoResponse> {
    let uid = Uuid::parse_str(&c.sub).map_err(|_| AppError::Unauthorized)?;
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let valid = ["contact", "company", "opportunity"];
    if !valid.contains(&r.entity_type.as_str()) {
        return Err(AppError::Validation("Invalid entity_type".into()));
    }
    sqlx::query_as::<_, Tag>("SELECT * FROM tags WHERE id=$1 AND tenant_id=$2")
        .bind(r.tag_id)
        .bind(t)
        .fetch_optional(&s.db)
        .await?
...
pub async fn unassign_tag(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| AppError::Unauthorized)?;
    let a = sqlx::query_as::<_, TagAssignment>(
        "SELECT * FROM tag_assignments WHERE id=$1 AND tenant_id=$2",
    )
    .bind(id)
    .bind(t)
    .fetch_optional(&s.db)
    .await?
    .ok_or(AppError::NotFound(format!("Assignment {id} not found")))?;
    sqlx::query("DELETE FROM tag_assignments WHERE id=$1 AND tenant_id=$2")
        .bind(id)
        .bind(t)
        .execute(&s.db)
        .await?;
