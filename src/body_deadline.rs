//! Request-body read deadline (kanban t_59745689).
//!
//! CoreSwift-CRM is the hub that RECEIVES lead pushes, so its inbound receivers are the routes a
//! stranger can reach: `/inbound/:key_prefix/:event_type` (and `/v2`, `/v3`),
//! `/api/v1/webhooks/cross-app/tag-sync`, `/api/webhook/:token/:action`,
//! `/api/external/contacts`, the `x-internal-key` `/api/internal/*` push routes,
//! `/api/public/contact`, `/api/messages/webhook`, `/api/v1/internal/tag-provision`,
//! `/api/v1/webhooks/mailgun/inbound`, `/api/telnyx/webhook`, `/api/telnyx/sms-webhook`,
//! `/api/widgets/:tenant_slug/:widget_slug/submit`, the `/s/:tenant_id/*` support portal, the
//! `/api/public/bookings` and `/api/auth/*` routes — and every one of them reads a request body.
//! Before this module a client could send a request head with a valid `Content-Length` and then
//! stop: the task, the connection and the partially-read body buffer stayed pinned for ever. No
//! 408, no close, no log line. Measured live on this app's deployed binary `70db4c31b7358a60`
//! (before/run.txt): 14 of 14 stalled-body legs parked with no response inside a 45 s budget,
//! including the lead-push receivers and a protected `POST /api/contacts` carrying a real session.
//! WorkflowSwift closed the same hole in t_e7cba83e (408 at t+30.0 s), ADASwift in t_b3d626ed and
//! FunnelSwift in t_488a19d4; this is the CoreSwift-CRM arm of that fleet-wide bound.
...
///
/// 30 s is orders of magnitude above the time a real body on these routes takes — a lead push or a
/// Telnyx event is kilobytes over a same-region link — and still generous to a slow sender: a full
/// 2 MiB body (axum's default limit, which this app does not raise anywhere) may arrive as slowly
/// as ~70 KiB/s and complete inside it.
...
///
/// **Why a body deadline and not a handler budget** (kanban t_59745689). The routes this is mounted
/// on do their own credential or key work *after* the body has been read: the `/inbound` receivers
/// look up their key prefix, `/api/external/contacts` resolves an API key, the `/api/internal/*`
/// routes check `x-internal-key` inside the handler, `/api/auth/login` hashes a password. A
/// wall-clock budget over those routes would have to be sized for that work plus whatever the
/// handler dispatches, and elapsing it drops work that was legitimately in progress — for a lead
/// push that is a LOST LEAD, not a retried login. This middleware bounds only the thing that is
/// actually unbounded: a client that sends a request head and then stops. A handler that
...
/// request is answered `408` and closed, and CoreSwift's inbound producers (FunnelSwift, ADASwift,
/// n8n) retry their pushes — at that point the sender had stopped mid-body, so the event was
...
pub async fn body_read_deadline_middleware(
    ...
    // not a claim, and it is why the handful of bodyless module routers can carry the layer without
    // their contract changing (proven live: a declared-but-absent body on GET /api/health answers
    // 200 at t+0.02 s in both phases, and a stalled body on a mounted bodyless router is answered
    // at once).
    ...
    // `Claims` an outer `auth_middleware` injected available to the handler.
...
mod tests {
    ...
    // ── body-read deadline (kanban t_59745689) ──────────────────────────────────────────────
    //
    ...
    // inbound receivers parse the raw JSON, so anything else silently breaks lead capture); and the
    ...
    /// ever — measured live on `/inbound/v3/...` at t+45.0 s and still parked.
    #[tokio::test]
    ...
    /// the body instead of replacing it: the inbound receivers and the internal push routes parse
    /// exactly these bytes into contacts, tags and tenant lookups.
    #[tokio::test]
    async fn complete_body_reaches_the_handler_unchanged() {
        let app = body_deadline_app(std::time::Duration::from_secs(30));
        let payload = br#"{"event_type":"contact-sync","data":{"email":"a@b.test"}}"#;
        let resp = app
            .oneshot(
                HttpRequest::post("/webhook")
    ...
    /// the scope leg — it is what lets the bodyless routers carry the layer without their contract
    /// changing, and it is measured live in both phases (probe legs N1/N2, S1).
    #[tokio::test]
