...
pub async fn tag_usage(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| crate::errors::AppError::Unauthorized)?;
    Ok(Json(
        json!({"tags": sqlx::query_as::<_,(Uuid,String,String,i64)>(
...
pub async fn contact_stats(
    ...
    let t = Uuid::parse_str(&c.aid).map_err(|_| crate::errors::AppError::Unauthorized)?;
    let total: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM contacts WHERE tenant_id=$1 AND is_active=true")
            .bind(t)
            .fetch_one(&s.db)
            .await
            .unwrap_or(0);
    ...
    let pct = if total > 0 {
        (with_tags as f64 / total as f64 * 100.0 * 100.0).round() / 100.0
    } else {
        0.0
    };
    Ok(Json(
        json!({"total_contacts": total, "contacts_with_opportunities": with_opps, "contacts_with_tags": with_tags, "tagged_percentage": pct}),
    ))
}
