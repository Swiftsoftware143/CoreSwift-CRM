#!/bin/bash
# deploy-from-vps.sh — coreswift-crm build & deploy with centralized build guard
set -e

cd /opt/swift/coreswift

echo "=== coreswift-crm Deploy: $(date) ==="

# --- Acquire build mutex ---
GUARD="/opt/swift/scripts/swift-build-guard.sh"
GUARD_RESULT=$("$GUARD" coreswift 2>&1) || {
  GUARD_EXIT=$?
...
# --- Health check ---
echo "=== Health check ==="
systemctl --no-pager status coreswift-crm | head -10
curl -sf localhost:8084/api/health && echo "" || echo "WARNING: Health check failed"
